...
Plain Markdown in CodeMirror 6 is the only document model. We conceal syntax tokens visually and apply typography directly over the source text. You get WYSIWYG ergonomics while keeping 100% byte fidelity with disk and Git.
...
4. **Distraction-free:** Minimal chrome, a `78ch` measure, and immersive writing modes.
5. **Fast at scale:** Viewport-scoped decorations plus incremental Lezer parsing.

---

## Features

- **Hybrid Visual Mode (`Cmd+1` / `Ctrl+1`)**: Live hiding of syntax tokens (`**bold**`, `*italic*`, `~~strike~~`, `` `code` ``, `[links](url)`, `# headings`) with interactive Notion-style tables, bullet dots (`•`), checkboxes, and Mermaid diagrams.
...
- **Floating Selection Toolbar**: A selection bubble for bold, italic, strikethrough, headings, and lists.
...
- **Adaptive Themes**: Light and dark themes driven by CSS custom properties.
...
3. Keep `src/core/` free of React imports.
